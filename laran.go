...
func main() {
	...
	// Get type to filter and support multiple comma-separated types
	typ := *typeFlag
	if typ != "" {
		// Split the input by comma to support multiple types
		types := strings.Split(typ, ",")
		for _, t := range types {
			t = strings.TrimSpace(t)
			// Process specified type for each provided value
			categoryName, ok := typeToCategory[t]
			if !ok {
